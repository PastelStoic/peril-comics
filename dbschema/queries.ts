...
export async function createComic(client: Executor, args: {
  ...
  return client.querySingle(`insert Comic {
  title := <str>$title,
...
export async function createCloudflareImage(client: Executor, args: {
  ...
  return client.querySingle(`insert CloudflareImage {
  image_id := <str>$id,
  image_name := <str>$name,
} 
...
export async function searchTags(client: Executor, args: {
  ...
  display_name,
  ref_name,
} filter ((<optional str>$name ilike .display_name) if exists <optional str>$name else true)`, args);
}
...
export async function addImageToComic(client: Executor, args: {
  ...
  return client.querySingle(`with usedImage := (select CloudflareImage filter .id = <uuid>$imageId)
insert ComicImage {
